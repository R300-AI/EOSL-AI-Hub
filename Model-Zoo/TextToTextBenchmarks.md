...
量化等級以 `qX`表示，其中 X 為位元數，例如 `q2` 為 2-bit 量化，壓縮率高但精度可能下降；`q4` 為 4-bit，屬於常見的平衡選擇；`q8` 則接近原始精度但速度較慢。
命名後綴如 K、K_S、K_M、K_L 則代表不同的 GPTQ 策略：K 為標準策略，K_S 為 Small（速度快、資源占用低）、K_M 為 Medium（平衡型）、K_L 為 Large（精度高但資源消耗大）。
